...
class BaselineFeatureExtractor:  # {{{
    ...
    def extract(self, parsed, deps, i, sent=None):
        ...
        f1_childs = children(f1)
        f1_id = f1['id']
        f1vr = 0
        f1vl = 0
        for child in f1_childs:
            if child['id'] > f1_id: f1vr += 1
            if child['id'] < f1_id: f1vl += 1
        f2_childs = children(f2)
        f2_id = f2['id']
        f2vr = 0
        f2vl = 0
        for child in f2_childs:
            if child['id'] > f2_id: f2vr += 1
            if child['id'] < f2_id: f2vl += 1
        append("f1wvr_%s_%s" % (f1_form, f1vr))
        append("f1tvr_%s_%s" % (f1_tag, f1vr))
        append("f1wvl_%s_%s" % (f1_form, f1vl))
        append("f1tvl_%s_%s" % (f1_tag, f1vl))
        append("f2wvr_%s_%s" % (f2_form, f2vr))
        append("f2tvr_%s_%s" % (f2_tag, f2vr))
        append("f2wvl_%s_%s" % (f2_form, f2vl))
        append("f2tvl_%s_%s" % (f2_tag, f2vl))
